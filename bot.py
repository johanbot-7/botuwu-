...
async def manejar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in estado_reportes:
        texto_problema = update.message.text.strip()
        if not texto_problema:
            await update.message.reply_text("❌ Ingresa texto válido")
            return

        todos_reportes.append(f"👤 @{update.effective_user.username} ({uid}): {texto_problema}")  # Guardar en lista global
...
# 🔹 USUARIO IDEAS
    if uid in estado_ideas:
        texto_idea = update.message.text.strip()
        if not texto_idea:
            await update.message.reply_text("❌ Ingresa texto válido")
            return

        todas_ideas.append(f"💡 @{update.effective_user.username} ({uid}): {texto_idea}")  # Guardar en lista global
    ...
    # 🔥 SOPORTE (ARreglado)
    if uid in estado_soporte:
        problema = texto

        if not problema:
            await update.message.reply_text("❌ Envía texto válido")
            return
...
"""
            ...
            # ✅ BOTÓN
            botones = [
                [InlineKeyboardButton("⚠️ Problemas", callback_data="reportar_problema")]
            ]
    ...
    if texto == "21":
        ...
        botones = [[
        InlineKeyboardButton("🟢 ON", callback_data="mantenimiento_on"),
        InlineKeyboardButton("🔴 OFF", callback_data="mantenimiento_off")
    ]]
